"""
Exploration des donnees meteorologiques historiques MeteoSuisse
===============================================================
Lit les fichiers meteo_*_historical.parquet generes par acquisition.py
Genere : data/eda_meteo.html

Dependances : pip install polars plotly pyarrow
Execution   : python eda_meteo.py
"""
...
# ── Colonnes historiques attendues ────────────────────────────────────────────
HIST_COLS = {
...
# ── Chargement ────────────────────────────────────────────────────────────────
def load_station(name: str) -> pl.DataFrame | None:
    """Charge le fichier historical d'une station."""
    # Chercher les deux formats possibles (avec ou sans _historical)
    candidates = [
        DATA_DIR / f"meteo_{name}.parquet",
        DATA_DIR / f"meteo_{name}_historical.parquet",
    ]
    for p in candidates:
        if p.exists():
            df = pl.read_parquet(p)
            # Garder seulement timestamp + colonnes hist_*
            hist_available = [c for c in df.columns
                              if c == "timestamp" or c.startswith("hist_")]
            if len(hist_available) > 1:
                return df.select(hist_available).sort("timestamp")
    return None


def discover_stations() -> dict[str, pl.DataFrame]:
    """Detecte automatiquement tous les fichiers meteo disponibles."""
    stations = {}
    for p in sorted(DATA_DIR.glob("meteo_*.parquet")):
        # Ignorer les fichiers de predictions
        ...
        name = (p.stem
                ...
                .replace("_", " ")
                .title())
        stations[name] = df.select(
            ["timestamp"] + hist_cols
        ).sort("timestamp")
        print(f"  {name:<30} {df.shape[0]:>9,} lignes  "
...
def fig_coverage(stations: dict) -> go.Figure:
    """Gantt de couverture temporelle par station et par variable."""
    fig = go.Figure()
    colors = ["#2E75B6","#ED7D31","#FFC000","#27AE60","#E74C3C","#9B59B6","#1ABC9C"]

    for i, (station, df) in enumerate(stations.items()):
        color = colors[i % len(colors)]
        # Calculer la couverture mensuelle (% de valeurs non-nulles)
...
def fig_nan_heatmap(stations: dict) -> go.Figure:
    """Heatmap NaN : stations x variables."""
    ...
    for station, df in stations.items():
        ...
        for col in var_cols:
            if col in df.columns:
                pct = df[col].is_null().sum() / df.shape[0] * 100
            else:
                pct = 100.0
            row.append(pct)
        z.append(row)
...
def fig_variable_overview(stations: dict, col: str) -> go.Figure:
    """Vue globale d'une variable sur toutes les stations (resample quotidien)."""
    ...
    for i, (station, df) in enumerate(stations.items()):
        if col not in df.columns:
            print(f"    [{station}] colonne {col} absente - ignoree")
            continue
...
def fig_radiation_night_anomaly(stations: dict) -> go.Figure:
    """Detecte le rayonnement > 0 pendant la nuit (anomalie potentielle)."""
    fig = go.Figure()
    colors = ["#FFC000","#ED7D31","#E74C3C","#9B59B6","#27AE60","#2E75B6","#1ABC9C"]
    ...
    for i, (station, df) in enumerate(stations.items()):
        ...
        rad_night = night["hist_radiation"].drop_nulls()
        n_nonzero = (rad_night > 0).sum()
        pct = n_nonzero / len(rad_night) * 100 if len(rad_night) > 0 else 0
        summary_lines.append(f"  {station:<25} {n_nonzero:>6,} pts > 0  ({pct:.2f}%)")

        monthly_anom = night.group_by_dynamic("timestamp", every="1mo").agg(
            (pl.col("hist_radiation") > 0).sum().alias("n_anom")
        ).sort("timestamp").to_pandas()

        fig.add_trace(go.Bar(
            x=monthly_anom["timestamp"],
            y=monthly_anom["n_anom"],
            name=station,
    ...
    apply_base(fig, "Anomalie : rayonnement > 0 la nuit (22h-3h) — nb points/mois", height=380)
    fig.update_layout(barmode="group", xaxis_title="Mois",
                      yaxis_title="Nb de points aberrants")
    return fig


def fig_outliers_boxplot(stations: dict) -> go.Figure:
    """Boxplots des variables cles pour toutes les stations."""
    # Utiliser seulement les colonnes presentes dans toutes les stations
    all_cols = ["hist_temperature", "hist_radiation", "hist_wind_speed", "hist_precipitation"]
    key_cols = [c for c in all_cols if any(c in df.columns for _, df in stations.items())]
    key_labels = ["Temperature [°C]", "Radiation [W/m²]", "Vent [m/s]", "Precip. [mm]"]
    colors = ["#2E75B6","#ED7D31","#FFC000","#27AE60","#E74C3C","#9B59B6","#1ABC9C"]

    fig = make_subplots(rows=1, cols=len(key_cols),
                        subplot_titles=key_labels)
    for j, col in enumerate(key_cols, 1):
        for i, (station, df) in enumerate(stations.items()):
            if col not in df.columns:
                continue
            vals = df[col].drop_nulls()
            # Echantillon pour la perf
            if len(vals) > 50000:
                vals = vals.sample(50000, seed=42)
    ...
    fig.update_layout(**LAYOUT_BASE, height=440,
        title=dict(text="Distribution des variables cles par station", x=0.01,
...
def fig_seasonal_profiles(stations: dict) -> go.Figure:
    """Profils saisonniers : temperature et radiation par mois."""
...
def fig_daily_profile(stations: dict) -> go.Figure:
    """Profil horaire moyen : radiation et temperature."""
    fig = make_subplots(rows=1, cols=2,
...
def fig_correlation_matrix(df: pl.DataFrame, station_name: str) -> go.Figure:
    """Matrice de correlation entre toutes les variables d'une station."""
    hist_cols = [c for c in df.columns if c.startswith("hist_")]
    ...
    apply_base(fig,
        f"Matrice de correlation entre variables — {station_name}", height=420)
...
def build_anomaly_report(stations: dict) -> str:
    ...
    for station, df in stations.items():
        ...
        for c in hist_cols:
            ...
            flag = " ⚠" if n > 0 else " ✓"
            lines.append(f"  {lbl:<28} {n:>8,} NaN  ({pct:5.2f}%){flag}")
...
if __name__ == "__main__":
    ...
    if not stations:
        print("\nAucun fichier meteo_*_historical.parquet trouve dans data/")
        print("Verifier que acquisition.py a bien ete execute.")
        exit(1)

    print(f"\n{len(stations)} station(s) chargee(s) : {list(stations.keys())}")

    # Station principale pour la matrice de correlation
    ...
    sections = [
        ...
        ("5. Precipitations — vue globale",
            fig_variable_overview(stations, "hist_precipitation")),
        ("6. Vitesse du vent — vue globale",
            fig_variable_overview(stations, "hist_wind_speed")),
        ("7. Anomalie : rayonnement nocturne > 0",
            fig_radiation_night_anomaly(stations)),
        ("8. Distribution et outliers par station",
            fig_outliers_boxplot(stations)),
        ("9. Profils saisonniers (temp. & radiation)",
            fig_seasonal_profiles(stations)),
        ("10. Profils horaires moyens",
            fig_daily_profile(stations)),
        (f"11. Matrice de correlation — {main_station_name}",
    ...
    print(f"\nExporte -> {out}  ({size_kb:.0f} KB)")
    print("Ouvrir dans un navigateur pour explorer les graphiques.")