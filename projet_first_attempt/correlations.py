...
def load_data() -> pl.DataFrame:
    ...
    df = df.drop([c for c in df.columns if c.endswith("_missing")])
...
def load_all_stations() -> dict[str, pl.DataFrame]:
    ...
    load_df     = df_oiken.select([c for c in ["timestamp", "load"]
                                   if c in df_oiken.columns])
    stations    = {}
    meteo_files = sorted([p for p in DATA_DIR.glob("meteo_*.parquet")
    ...
    for p in meteo_files:
        ...
        df_joined = df_joined.drop(
            [c for c in df_joined.columns if c.endswith("_right")]
        )
...
def fig_hourly_heatmap(df: pl.DataFrame, results: list[tuple]) -> go.Figure:
    ...
    fig = go.Figure(go.Heatmap(
        ...
        y=[short_name(c) for c in features],
...
def fig_load_autocorr(df: pl.DataFrame, max_lag_h: int = 168) -> go.Figure:
    ...
    for peak_h in peaks_of_interest:
        if peak_h <= max_lag_h:
            r_peak = corrs[peak_h]
            color  = ("#FFC000" if abs(r_peak) >= 0.5 else
                      "#F39C12" if abs(r_peak) >= 0.3 else "#8B949E")
            fig.add_trace(go.Scatter(
                x=[peak_h], y=[r_peak],
                mode="markers+text",
    ...
    apply_base(fig, "Autocorrelation du load — r(load[t], load[t-lag])  lag 0-168h",
               height=460)
    fig.update_layout(
...
def fig_seasonal(df: pl.DataFrame, results: list[tuple]) -> go.Figure:
    ...
    apply_base(fig, "Correlations par saison", height=420)
    fig.update_layout(
        barmode="group",
        xaxis_tickangle=-35,
...
def fig_lagged_all_stations(stations: dict[str, pl.DataFrame],
    ...
    apply_base(fig, f"Cross-correlation inter-stations — {var_label}  |  "
                    f"r(load[t+lag], meteo_station[t])  |  pic = avance geographique",
               height=480)
    fig.add_hline(y=0,    line_color=TEXT,      line_width=1, line_dash="dot")
    fig.add_hline(y=0.3,  line_color="#27AE60", line_width=1, line_dash="dot",
                  annotation_text="+0.3", annotation_font_color="#27AE60",
...
def fig_calendar_boxplots(df_main: pl.DataFrame,
    ...
    colors_g  = {"Normal":"#2E75B6","Ferie":"#E74C3C","Vacances scol.":"#FFC000"}
    ...
    fig.update_layout(
        **LAYOUT_BASE, height=700,
        title=dict(text="Variables calendaires vs Load",
...
def fig_feature_corr_matrix(df: pl.DataFrame,
    ...
    for i in range(n):
        for j in range(i+1, n):
            if abs(matrix[i][j]) > 0.85:
                high_corr_pairs.append((labels[i], labels[j], matrix[i][j]))
    ...
    apply_base(fig,
        f"Matrice de correlation entre features — {len(high_corr_pairs)} paires |r|>0.85",
...
<h1>Correlations avec le Load</h1>
<div class="subtitle">
...
if __name__ == "__main__":
    ...
    sections = [
        ("1. Correlations globales — Sion",
            fig_global(results)),
        ("2. Heatmap horaire — r par heure de la journee",
            fig_hourly_heatmap(df, results)),
        ("3. Autocorrelation du load — lag 0-168h",
            fig_autocorr),
