"""
Visualisation sliding window golden — LightGBM + Optuna
=========================================================
Lit  : golden_data/sw_results_lgb_golden.parquet
       golden_data/sw_predictions_lgb_golden.parquet
       golden_data/sw_hyperparams_lgb_golden.parquet
Ecrit: golden_data/visualisation_sw_lgb_golden.html
"""
...
FILE_OUT   = GOLDEN_DIR / "visualisation_sw_lgb_golden.html"
...
if __name__ == "__main__":
    ...
    results = pl.read_parquet(GOLDEN_DIR / "sw_results_lgb_golden.parquet")
    preds   = pl.read_parquet(GOLDEN_DIR / "sw_predictions_lgb_golden.parquet")
    params  = pl.read_parquet(GOLDEN_DIR / "sw_hyperparams_lgb_golden.parquet")
    ...
    all_months = sorted(results["test_month"].to_list())
    ...
    mae_oiken_dict = {}
    for row in (
        preds.filter(pl.col("oiken_valid") == True)
        .with_columns([
...
<head>
<meta charset="UTF-8">
<title>SW LightGBM — Golden dataset</title>
...
.month-badge {{ display:inline-block; padding:2px 8px; border-radius:4px;
                font-size:10px; background:#1a1225; color:#9B59B6;
                margin-left:8px; }}
...
<h1>Sliding window LightGBM — Golden dataset</h1>
<div class="subtitle">
  22 mois train | 4 mois val (Optuna 30 essais) | 1 mois test |
  7 iterations golden (oct 2025 → avr 2026) | features v6
</div>
...
<div class="layout">
  <div class="left">
    ...
    <!-- MAE par mois -->
    <div class="card">
      <div class="card-title">MAE par mois — LightGBM vs Oiken (golden)</div>
      <div id="chart-monthly" style="height:280px;"></div>
    </div>

    <!-- Hyperparametres -->
    <div class="card">
      <div class="card-title">Hyperparametres Optuna</div>
      <div id="chart-hp" style="height:250px;"></div>
  ...
  <div class="right">
    ...
    <!-- Stats globales -->
    <div class="card">
      <div class="card-title">Resume global — golden</div>
      <div id="global-stats"></div>
    </div>

    <!-- Detail iteration -->
    <div class="card">
      <div class="card-title">Detail iteration</div>
      <div id="iter-detail" style="font-size:11px;color:{SUBTEXT}">
        Naviguez vers un jour pour voir le detail
      </div>
    </div>
...
function renderAll() {{
  ...
  document.getElementById("period-label").textContent =
    currentMode==="day" ? "— "+dates[0] :
    currentMode==="week" ? "— semaine" : "— mois";

  let testMonth = DAY_DATA[dates[0]]?.test_month || "";
  document.getElementById("month-badge").textContent = "test " + testMonth;
  ...
  let traces = [
    ...
    {{ x:allTs, y:allPred, name:"LightGBM SW",
...
function renderMonthlyChart() {{
  ...
  Plotly.react("chart-monthly", [
    ...
    {{
      x:months, y:maeModel, name:"LightGBM SW",
...
function renderCalendar() {{
  ...
  for(let day=1;day<=daysInM;day++) {{
    ...
    if(!info) {{
      ...
      html+=`<td style="background:#1a1225" class="${{sel}}"
