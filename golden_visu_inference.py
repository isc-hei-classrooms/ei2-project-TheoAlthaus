"""
Visualisation inference golden — v6
=====================================
Lit  : golden_data/predictions_v6_golden.parquet
       golden_data/oiken_golden_clean.parquet
Ecrit: golden_data/visualisation_v6_golden.html
"""
...
FILE_OUT   = GOLDEN_DIR / "visualisation_v6_golden.html"
...
if __name__ == "__main__":
    ...
    df_pred = pl.read_parquet(GOLDEN_DIR / "predictions_v6_golden.parquet")
    ...
    all_months  = sorted(df_pred["month_str"].unique().to_list())

    print(f"  Modeles : {models_list}")
    print(f"  Jours   : {len(all_dates)}")
    print(f"  Mois    : {all_months}")
    ...
    for row in (
        ...
        .group_by("date_str").agg(pl.col("err").mean().alias("mae_oiken"))
    ...
    for model_name in models_list:
        ...
        for row in (
            ...
            .group_by("date_str").agg(pl.col("err").mean().alias("mae"))
            .iter_rows(named=True)
        ):
    ...
    for model_name in models_list:
        ...
        for row in (
            ...
            monthly_stats[model_name][row["month_str"]] = {
                "mae_model": round(row["mae_model"], 5),
                "mae_oiken": round(row["mae_oiken"], 5),
                "n_pts":     row["n_pts"],
                "diff_pct":  round((row["mae_model"] - row["mae_oiken"])
                                   / row["mae_oiken"] * 100, 1),
            }
...
<head>
<meta charset="UTF-8">
<title>Golden dataset — inference v6</title>
...
.month-btn {{ background:#21262D; border:1px solid #30363D; color:{SUBTEXT};
              padding:4px 10px; border-radius:4px; cursor:pointer;
              font-size:10px; margin:2px; }}
.month-btn.active {{ background:#2E75B6; border-color:#2E75B6; color:{TEXT}; }}
</style>
</head>
<body>

<h1>Golden dataset — Inference v6</h1>
<div class="subtitle">
  XGBoost + LightGBM | evaluation sur donnees inedites 2025-09-30 → 2026-04-22 |
  comparaison Oiken forecast
</div>
...
<div class="layout">
  <div class="left">
    ...
    <div class="card">
      <div class="card-title">Comparaison MAE par mois</div>
  ...
  <div class="right">
    ...
    <!-- Stats globales -->
    <div class="card">
      <div class="card-title">Stats globales — golden</div>
...
function navigate(dir) {{
  if (currentMode === "month") {{
    ...
    let curMonth = allDates[currentIdx].substring(0,7);
    let idx = months.indexOf(curMonth);
    let newIdx = Math.max(0, Math.min(months.length-1, idx+dir));
    let newMonth = months[newIdx];
    let firstDate = allDates.find(d => d.startsWith(newMonth));
    if (firstDate) currentIdx = allDates.indexOf(firstDate);
  }} else {{
...
function renderAll() {{
  ...
  if (currentMode === "week")  renderWeek(currentIdx);
  if (currentMode === "month") renderMonth(allDates[currentIdx].substring(0,7));
}}
...
function renderWeek(startIdx) {{
  ...
  renderMultiDay(dates);
  renderMetrics(dates);
}}

function renderMonth(monthStr) {{
  let dates = allDates.filter(d => d.startsWith(monthStr));
  document.getElementById("date-label").textContent   = monthStr;
  document.getElementById("period-label").textContent = "— mois complet";
  renderMultiDay(dates);
  renderMetrics(dates);
}}
...
function renderMonthlyChart() {{
  ...
  traces.push({{
    x: months,
    y: months.map(m => MONTHLY_STATS[BEST_MODEL]?.[m]?.mae_oiken ?? null),
    name: "Oiken forecast",
  ...
  MODELS_LIST.forEach(model => {{
    traces.push({{
      x: months,
      y: months.map(m => MONTHLY_STATS[model]?.[m]?.mae_model ?? null),
      name: model,
      line: {{color:MODEL_COLORS[model]||"#888", width:2}},
      type:"scatter", mode:"lines+markers",
      marker:{{size:6}},
    }});
  }});
...
function renderGlobalStats() {{
  ...
  MODELS_LIST.forEach(model => {{
    ...
    allDates.forEach(d => {{
      ...
      trueAll = trueAll.concat(data.true);
      fcAll   = fcAll.concat(data.forecast);
      predAll = predAll.concat(mm[model] || []);
    }});
    let maeM = mae(trueAll, predAll);
    let maeO = mae(trueAll, fcAll);
