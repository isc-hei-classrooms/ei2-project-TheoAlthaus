"""
Entrainement modeles — version 6
==================================
Lit  : data/features_v6.parquet
       data/oiken_clean.parquet
Ecrit: data/models/results_v6.parquet
       data/models/predictions_v6.parquet
       data/models/wf_results_v6.parquet
       data/models/wf_predictions_v6.parquet

Modeles : LightGBM, XGBoost (single-output)
Validation : simple + walk-forward (pas=30j)
Correction : exclusion periode Oiken buguee
...
if __name__ == "__main__":
    ...
    for name, model in [
        ...
        with open(MODELS_DIR / f"{name}_v6.pkl", "wb") as f:
            pickle.dump(model, f)
    ...
    pl.concat(simple_preds).write_parquet(MODELS_DIR / "predictions_v6.parquet")
    pl.DataFrame(simple_metrics).write_parquet(MODELS_DIR / "results_v6.parquet")
    ...
    pl.concat(wf_preds).write_parquet(MODELS_DIR / "wf_predictions_v6.parquet")
    pl.DataFrame(wf_metrics).write_parquet(MODELS_DIR / "wf_results_v6.parquet")

    # ── Resume final ──────────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print("RESUME FINAL — v6")
    print(f"{'='*60}")
    ...
    for p in sorted(MODELS_DIR.glob("*v6*")):
