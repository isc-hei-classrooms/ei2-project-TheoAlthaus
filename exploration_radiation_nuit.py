"""
Analyse du rayonnement nocturne par station
============================================
Genere : data/eda_radiation_nuit.html

Dependances : pip install polars plotly pyarrow
Execution   : python eda_radiation_nuit.py
"""
...
# ── 1. Distribution des valeurs nocturnes (histogramme + stats) ──────────────
def fig_night_histogram(stations: dict) -> go.Figure:
    """Histogramme des valeurs de rayonnement entre 22h et 4h."""
    fig = make_subplots(
        rows=2, cols=4,
    ...
    for i, (name, df) in enumerate(stations.items()):
        ...
        fig.add_annotation(
            ...
            xref=f"x{'' if (row==1 and col==1) else i+1}",
            yref=f"y{'' if (row==1 and col==1) else i+1}",
            x=0.98, y=0.98,
...
# ── 2. Comparaison nuit vs jour (boxplot superpose) ───────────────────────────
def fig_night_vs_day(stations: dict) -> go.Figure:
    """Boxplot cote a cote : valeurs nocturnes vs diurnes par station."""
    fig = go.Figure()
...
# ── 3. Serie temporelle nocturne sur quelques jours ───────────────────────────
def fig_night_timeseries(stations: dict) -> go.Figure:
    """Serie temporelle zoom sur 5 jours pour voir le comportement reel."""
    import datetime, pytz

    # Prendre une semaine d'hiver (peu de soleil, anomalie plus visible)
...
"""]
    ...
    # Tableau de synthese en premier
    html_parts.append(f"""
<div class="section">
...
if __name__ == "__main__":
    ...
    if not stations:
        print("Aucun fichier meteo_* avec hist_radiation trouve dans data/")
        exit(1)
    ...
    print("Calcul du tableau de synthese...")
    ...
    print(f"\nExporte -> {out}  ({out.stat().st_size/1024:.0f} KB)")
